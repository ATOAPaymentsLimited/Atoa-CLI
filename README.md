...
Detects your CPU (x64/arm64), verifies the download's checksum and GPG signature, and puts `atoa` on
your PATH. Pin a version or location with `ATOA_VERSION` / `ATOA_INSTALL_DIR`. Prefer to download
manually? Grab `atoa-linux-x64` (or `atoa-linux-arm64`) from the
[latest release](https://github.com/ATOAPaymentsLimited/Atoa-CLI/releases/latest) and verify it with
the bundled `SHA256SUMS` / `SHA256SUMS.asc`.

> The prebuilt binaries are **glibc-only**. On Alpine/musl, use the npm install above.

### Windows (standalone binary, no Node.js)
...
Verifies the checksum and Authenticode signature, then adds `atoa` to your PATH.

### macOS

Use the **npm** install above for now. Signed, notarized macOS binaries will be published once Apple
notarization is enabled in the release pipeline.
